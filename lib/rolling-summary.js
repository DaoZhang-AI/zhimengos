...
export const DEFAULTS = {
    ...
    keepRaw: 40,
    /** 原话比保留数多出这么多条,就把最老的这么多条压成一段摘要,压完又回到 keepRaw 条。
     *  小步走(道长 9/18:选了 40,到 48 就把 1-8 压掉),原来一次攒 60 条太粗 */
    batchSize: 8,
    /** 摘要自己也会越攒越多:超过这么多段,就把最老的 compactBatch 段合成一段。
     *  界面上不给设,自动的,不和上面两个数打架 */
    compactAfter: 12,
    /** 压的时候一次合并最老的这么多段 */
    compactBatch: 6,
};

// 摘要和聊天用同一个模型(道长 9/18),所以"老实"全靠这套要求管。
// 聊得好的模型写摘要爱顺手编,最常见的是补动机、补心理、补没发生的后续,下面逐条堵。
const HONEST = [
    '你现在不是在聊天,是在做记录。你不扮演任何人。',
    '',
    '铁律:',
    '- 只写聊天里**明明白白发生了**的事。聊天里没有的,一个字都不许加。',
    '- 不补动机、不猜心理、不替谁解释"其实是因为"。聊天里没说为什么,就不写为什么。',
    '- 不写后续。聊天停在哪,摘要就停在哪。',
    '- 人名、称呼、东西的名字照聊天里的原样写,不许换成别的叫法。',
    '- 拿不准的宁可不写。短一点没关系,编出来的比漏掉的害处大得多。',
    '',
].join('\n');
...
const ASK_COMPACT = [
    HONEST,
    '下面是几段按时间先后排列的摘要。请把它们合并成一段更短的。只能从这几段里取,不许加这几段里没有的东西。',
    '',
...
function needsSummary(store, opts) {
    // 保留 40、每次 8:到第 48 条就压 1-8,剩 40(道长 9/18 说的就是这个节奏)
